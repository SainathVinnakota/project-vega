...
class CoactionCrawler(BaseCrawler):
    ...
    def chunk_class_code_page(self, text: str, url: str, code: str) -> list[dict]:
        ...
        if len(full_text) <= 8000:
            return [{"text": full_text, "metadata": {"source_url": url, "heading": prefix, "class_code": code, "class_name": class_name, "chunk_type": "class_code_full"}}]
        sections = re.split(r'\n(?=# )', text)
        chunks = []
        for section in sections:
            ...
            chunks.append({"text": f"{prefix}\n{heading_text}\n\n{section}", "metadata": {"source_url": url, "heading": f"{prefix} — {heading_text}", "class_code": code, "class_name": class_name, "chunk_type": "class_code_section"}})
        return chunks or [{"text": full_text[:8000], "metadata": {"source_url": url, "heading": prefix, "class_code": code, "class_name": class_name, "chunk_type": "class_code_full"}}]
    ...
    def chunk_guide_page(self, text: str, url: str) -> list[dict]:
        ...
        for section in sections:
            ...
            if len(section) <= 6000:
                chunks.append({"text": section, "metadata": {"source_url": url, "heading": heading, "chunk_type": "guide_section"}})
            else:
                ...
                for para in paragraphs:
                    if len(current) + len(para) <= 6000:
                        ...
                        if current.strip() and len(current.split()) >= 20:
                            chunks.append({"text": current.strip(), "metadata": {"source_url": url, "heading": heading, "chunk_type": "guide_section"}})
                        current = para
                if current.strip() and len(current.split()) >= 20:
                    chunks.append({"text": current.strip(), "metadata": {"source_url": url, "heading": heading, "chunk_type": "guide_section"}})
        return chunks
